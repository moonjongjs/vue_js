...
function chkText(field,text,msg)
{
	...
	if (text==""){
        ...
        if(field.name == 'mobileOrder[]'){ // 주문서 보내는분
            $('[name=mobileInp]').focus();
        }else if(field.name == 'mobileReceiver[]'){ // 주문서 받는분
            $('[name=mobileReceiverInp]').focus();
        }else{
...
function chkSelect(form,field,msg)
{
	...
	if (fieldname.length){
		for (var j=0;j<fieldname.length;j++) if (fieldname[j].checked) ret = true;
	} else {
...
function orderCntCalc(o, cnt, set) {
	...
	var min = parseInt(o.getAttribute('min')) || 0;
